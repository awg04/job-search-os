...
5. **Update the dashboard.** In `dashboard/job-hunt.html`, replace the content between the markers (keep the markers themselves):
   ...
   ```
   **Ref semantics:** a card's ref — what its Next Steps / Archive buttons pass and what `dashboard-state.json` keys its state by — is its **url**, falling back to `threadId` only when url is null. `threadId` is provenance (the source alert email, for the Open Email button), deliberately NOT the ref: one digest email contributes many cards sharing a threadId, and per-posting refs keep their archive/✓ state independent. So prefer a real per-posting link in `url` whenever the alert offers one.
